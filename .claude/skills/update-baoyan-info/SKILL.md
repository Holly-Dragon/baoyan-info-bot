...
- 写入完成后，确保腾讯共享表格中新增内容的字体统一为宋体
- **"通知链接"字段写入为超链接**：链接显示文本与 URL 地址相同，点击即可直接跳转
- **"申请倒计时"字段使用 Excel 公式实时计算**：
...
- **公式字段**：新写入的数据必须使用 Excel 公式计算"申请倒计时"字段，确保数据实时更新。
- **字体要求**：写入腾讯共享表格后，所有新增单元格内容的字体应统一为宋体。