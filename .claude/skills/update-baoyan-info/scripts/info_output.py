    ...
    try:
        ...
        for sheet_name, df in dfs.items():
            ...
            if target_col in headers:
                ...
                # 如果 Excel 最大行和 DataFrame 行数不一致，进行截断或补齐
...
def export_categorized_excel(data_list, output_file="2026院校信息_分类.xlsx"):
    ...
    for row in data_list:
        academy = row.get('学院', '')
        if '管理' in academy or '经济' in academy or "商学" in academy or "金融" in academy or "法" in academy:
...
def export_excel_to_word(excel_file="2026院校信息_分类.xlsx", word_file="2026院校信息_汇总.docx"):
    ...
    dfs = read_excel_preserving_links(excel_file)
    ...
    for sheet_name, df in dfs.items():
        ...
        for school, group in grouped:
            ...
            for _, row in group.iterrows():
                ...
                link = extract_url_from_hyperlink(link)
...
def merge_and_output(main_file="2026院校信息.xlsx", update_file="2026院校信息_更新.xlsx", output_categorized="2026院校信息_分类.xlsx"):
    ...
    main_file: 主文件路径，更新后的数据将合并写入此文件，用户不分类存储所有数据的主文件
    update_file: 更新文件路径，包含最新的增量数据
    output_categorized: 分类输出文件路径，基于合并后的主文件生成
    """
    ...
    if os.path.exists(main_file):
        dfs_main = read_excel_preserving_links(main_file)
        if len(dfs_main) > 0:
            df_main = list(dfs_main.values())[0] if 'Sheet1' not in dfs_main else dfs_main['Sheet1']
        else:
    ...
    dfs_update = read_excel_preserving_links(update_file)
    update_count = 0
    ...
    # 恢复主文件超链接函数
    df_merged['通知链接'] = df_merged['通知链接'].apply(lambda x: f'=HYPERLINK("{x}", "{x}")' if str(x).startswith('http') else x)
    ...
    export_excel_to_word(update_file, f"2026院校信息_{current_date_str}.docx")
