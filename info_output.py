...
def export_excel_to_word(excel_file="2026院校信息_分类.xlsx", word_file="2026院校信息_汇总.docx"):
    ...
    for sheet_name, df in dfs.items():
        ...
        for school, group in grouped:
            ...
            # 五号 = 10.5pt，加粗，蓝色，左对齐
            set_run_font(run_sch, '微软雅黑', 10.5, bold=True, color=RGBColor(0, 0, 255))
            ...
            for _, row in group.iterrows():
                ...
                if link and link != '无链接':
                    ...
                    add_hyperlink(p_link, link, link, color="0000FF", underline=True)
                ...
                if deadline:
                    ...
                    set_run_font(run_dl, '微软雅黑', 10.5, bold=False, color=RGBColor(0, 0, 255))
...
def merge_and_output(main_file="2026院校信息.xlsx", update_file="2026院校信息_更新.xlsx", output_categorized="2026院校信息_分类.xlsx"):
    ...
    # 5. 生成 Word 汇总文档
    export_excel_to_word(output_categorized, "2026院校信息_汇总.docx")
